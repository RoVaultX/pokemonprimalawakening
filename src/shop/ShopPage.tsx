    ...
    async function loadStock() {
      try {
        if (isLocalStockEnabled) {
          ...
          setStockRobux(0);
          setStockError("Local dev: open /admin and save stock (stored in this browser) or set VITE_WORKER_API_BASE.");
          return;
        }
        if (!workerApiBase) {
          setStockRobux(0);
          setStockError("Stock is loaded from the admin backend. Set VITE_WORKER_API_BASE to your Worker URL.");
          return;
        }
        const stockUrl = new URL(`${workerApiBase}/api/stock`, window.location.origin);
        ...
        const parsedStock = parseStockValue(data.stockRobux);
        if (parsedStock === null) {
          throw new Error("Stock response is missing a numeric stockRobux value.");
        }
        setStockRobux(parsedStock);
        setStockError("");
      } catch (error) {
